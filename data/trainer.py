...
def get_most_frequent_pair(pair_frequencies: Counter):
    if not pair_frequencies:
        return None
    return pair_frequencies.most_common(1)[0][0]


def train_bpe(tokenizer, text: str):
    word_freqs = Counter(re.findall(r"[a-z0-9]+|[^\w\s]", text.lower()))
    return train_bpe_from_word_frequencies(tokenizer=tokenizer, word_freqs=word_freqs)
...
def train_bpe_from_word_frequencies(
    ...
    pair_frequencies = count_weighted_pair_frequencies(word_token_ids, word_freqs)
    ...
    while next_token_id < tokenizer.vocab_size:
        ...
        for word in affected_words:
            ...
            new_ids = merge_pair(ids=old_ids, pair=pair, new_token_id=next_token_id)
        ...
        if next_token_id % 100 == 0:
            print(
                ...
                f"Pair: {pair} | "
                f"Frequency: {pair_frequencies.get(pair, 0)}"
            )
